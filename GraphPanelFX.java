...
/**
* GraphPanelFX creates the Canvas and
* draws the graph
*/
public class GraphPanelFX {
   ...
   GraphPanelFX(GraphManager graphManager, double CANVAS_WIDTH,
   ...
   /**
   * drawGraph is called when the "Graph a Function"
   * button is selected
   */
   public void drawGraph() {
       ...
       System.out.println("in paintComponent(); width="
   ...
   /**
   * Draws line segments from left extent to right extent,
   * pixel by pixel,transforming points to the
   * coordinate system of the panel.
   */
   public void drawGraph(double gridWidth2, double gridHeight,
           GraphicsContext gc)
   {
       ...
       /*
       * check to make sure a function choice
       * has been made before drawing
       */
       if (functionChoice > 0) {
           ...
           /*
           * loop pixel by pixel, drawing the function
           ...
           */
           for (int i = 1; i < gridWidth2; i++) {
               ...
               System.out.println("x0=" + x0 + " y0=" +
                       graphMgr.getFnValue(functionChoice, x0)
                       + "; x0Draw="+ x0Draw + " y0Draw=" + y0Draw);
               x0 = x1;
